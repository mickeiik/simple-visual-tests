...
import { type StoryIdentifier, type Theme } from "./src/types/types";
...
describe("basic", async () => {
  ...
  for (const entry of stories) {
    ...
    for (const theme of themes) {
      for (const key of Object.keys(testedViewports)) {
        const storybookViewport = testedViewports[key];
        const viewportSize = {
...
async function loadStories(): Promise<StoryIndexEntry[]> {
  ...
  const data: Record<string, StoryIndexEntry> = await response.json();
